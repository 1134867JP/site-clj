...
<head>
  ...
  <title>Definir nova senha — CifraDocs</title>
  ...
  <div class="grid min-h-[100svh] grid-cols-1 lg:grid-cols-2">
    <section class="relative hidden lg:block">
      ...
      <div class="absolute bottom-8 left-8 right-10">
        <h2 class="text-2xl font-semibold drop-shadow">Defina sua nova senha</h2>
        <p class="text-white/85">Crie uma senha forte para sua conta.</p>
      </div>
    </section>
    ...
    <main class="flex items-center justify-center p-6">
      <div class="w-full max-w-md">
        ...
        <div class="rounded-2xl bg-neutral-900/75 backdrop-blur-xl border border-white/10 shadow-2xl p-8">
          <h1 class="text-2xl font-bold mb-1">Nova senha</h1>
          <p class="text-white/80 mb-6">Preencha os campos abaixo.</p>
          ...
          <form method="POST" action="{{ route('password.store') }}" novalidate>
            ...
            <x-primary-button class="w-full justify-center mt-6">{{ __('Definir senha') }}</x-primary-button>
          </form>
        </div>
      </div>
    </main>
