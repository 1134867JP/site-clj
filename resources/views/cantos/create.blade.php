<x-app-layout>
  ...
  <div class="max-w-6xl mx-auto py-8 px-4">
    <div class="bg-white/70 dark:bg-neutral-900/70 backdrop-blur-xl shadow-2xl rounded-2xl border border-white/10 overflow-hidden">
      <div class="px-6 pt-6 pb-2 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          <a href="{{ route('cantos.index') }}"
             class="px-3 py-2 rounded-xl border border-white/10 bg-white/30 dark:bg-white/10 text-gray-800 dark:text-gray-100 hover:bg-white/40 dark:hover:bg-white/20 backdrop-blur">
            Cancelar
          </a>
      ...
      {{-- Erros de validação --}}
      @if ($errors->any())
        <div class="mx-6 mb-4 rounded-xl border border-red-400/30 bg-red-600/10 dark:bg-red-900/30 text-red-800 dark:text-red-200 p-4">
        ...
        <form id="form-canto" method="POST" action="{{ route('cantos.store') }}" class="space-y-5">
          ...
          <div>
            ...
            <select name="tom"
                    class="w-full rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500">
              <option value=""></option>
              @php $tomOld = old('tom'); @endphp
              @foreach (['C','C#','Db','D','D#','Eb','E','F','F#','Gb','G','G#','Ab','A','A#','Bb','B', 'B4'] as $t)
                <option value="{{ $t }}" @selected($tomOld===$t)>{{ $t }}</option>
              @endforeach
            </select>
            @error('tom')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
...
[Ebm] Deixa pra depois, pra quem merecer"
        ...
        <div class="mt-6 md:mt-0">
          <div class="rounded-2xl border border-white/10 bg-white/50 supports-[backdrop-filter]:bg-white/30 dark:bg-white/5 backdrop-blur p-4">
            ...
            <pre id="preview"
                 class="bg-white/30 dark:bg-white/5 p-4 rounded-lg text-gray-900 dark:text-gray-100 whitespace-pre-wrap font-mono leading-relaxed text-[14px] border border-white/10 shadow-inner min-h-[260px]"></pre>
          </div>
        </div>
      </div>
    </div>
